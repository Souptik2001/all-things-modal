 ...
 *
 * @package all-things-modal-featuers
 */

namespace All_Things_Modal\Features\Inc\Helpers;
...
function autoloader( $resource = '' ) {
	...
	$resource_path  = false;
	$namespace_root = 'All_Things_Modal\Features\\';
	...
	if ( 'inc' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/inc/%s/%s.php', untrailingslashit( ALL_THINGS_MODAL_FEATURES_PATH ), $directory, $file_name );
	...
	if ( 'tests' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/tests/php/%s.php', untrailingslashit( ALL_THINGS_MODAL_FEATURES_PATH ), $file_name );
...
spl_autoload_register( '\All_Things_Modal\Features\Inc\Helpers\autoloader' );
