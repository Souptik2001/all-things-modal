<?php
/**
 * All_Things_Modal features custom functions.
 *
 ...
 */
function all_things_modal_get_cache_key( $unique = '' ) {

	$cache_key = 'all_things_modal_cache_';
 ...
 */
function all_things_modal_get_cached_posts( $args ) {
	...
	$cache_key = all_things_modal_get_cache_key( $args );

	$cache  = new \All_Things_Modal\Features\Inc\Cache( $cache_key );
 ...
 */
function all_things_modal_get_template_content( $slug, $vars = [] ) {
 ...
 */
function all_things_modal_features_template( $template, $variables = [], $echo = false ) {

	$template_file = sprintf( '%1$s/templates/%2$s.php', ALL_THINGS_MODAL_FEATURES_PATH, $template );
 ...
 */
function all_things_modal_get_data( $slug, $default = [] ) {

	$data_file = sprintf( ALL_THINGS_MODAL_FEATURES_PATH . '/inc/data/%s.php', $slug );
 ...
 */
function all_things_modal_is_production() {
 ...
 */
function all_things_modal_is_mobile( $kind = 'any', $return_matched_agent = false ) {
