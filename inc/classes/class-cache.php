...
class Cache {
	 ...
	 * @var string
	 */
	const ERROR_CODE = 'all_things_modal_cache';
	 ...
	 * @var string
	 */
	protected static $cache_group = 'all_things_modal_cache_v1';
	...
	public function __construct( $cache_key = '', $cache_group = '' ) {
		...
		if ( empty( $cache_key ) || ! is_string( $cache_key ) ) {
			return new \WP_Error( self::ERROR_CODE, esc_html__( 'Cache key is required to create cache object', 'all-things-modal' ) );
		}
	...
	protected function init() {
		 ...
		 */
		$cache_group = apply_filters( 'all_things_modal_cache_group_override', self::$cache_group, $this->key );
	...
	public function updates_with( $callback, $params = [] ) {
		...
		if ( empty( $callback ) || ! is_callable( $callback ) ) {
			return new \WP_Error( self::ERROR_CODE, esc_html__( 'Callback passed is not callable', 'all-things-modal' ) );
		}

		if ( ! is_array( $params ) ) {
			return new \WP_Error( self::ERROR_CODE, esc_html__( 'All parameters for the callback must be in an array', 'all-things-modal' ) );
		}
	...
	public function get() {
		...
		if ( empty( $this->callback ) || ! is_callable( $this->callback ) ) {
			return new \WP_Error( self::ERROR_CODE, esc_html__( 'No valid callback set', 'all-things-modal' ) );
		}
